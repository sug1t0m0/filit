...
| `POST /api/bundles`        | `{ files: string[] }` を受け取り合成 Markdown を生成、`{ id, bytes }` を返す |
| `GET /bundle/:id`          | 合成結果を最小 HTML (`<pre>` 1 個) で返す。「タブに開く」の遷移先 (R6)       |
| `GET /api/heartbeat`       | SSE。クライアント切断後 N 秒でプロセス自動終了 (difit 方式)                  |

- 「タブに開く」は **POST で bundle 生成 → 返却された id で `window.open('/bundle/:id')`** とする。GET クエリにファイルリストを載せる方式は URL 長制限があるため採らない。
- bundle はメモリ保持 (プロセス生存中のみ)。永続化しない。
...
- 右ペイン: ツリーでファイル名をクリックすると内容表示。行ホバーで `[+]` → クリック/ドラッグで行・範囲コメント追加 (difit の CommentButton / CommentForm を移植)。
- ヘッダ: 選択ファイルの UTF-8 バイト合計をライブ表示 (ツリー API のサイズ合算 + コメント分)。「タブに開く」ボタン。
- シンタックスハイライトは **MVP ではなし** (プレーン表示)。難所を減らし、後で difit の Prism 構成を移植する。
...
- 先頭にファイル数・合計バイト数のヘッダ行を置き、LLM に全体像を伝える。
- `/bundle/:id` ページはこの Markdown を `<pre>` に入れただけの最小 HTML (NF4)。装飾 CSS なし。
